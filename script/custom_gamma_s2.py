"""
Custom Gamma Menu Module for Stream 2
Для управления окном редактора Custom Gamma для Stream 2
"""
...
def open_custom_gamma_menu_s2(app):
    """
    Открыть окно Custom Gamma для Stream 2
    
    Args:
        app: Экземпляр приложения GPUCaptureApp
    """
    # Проверка, открыто ли окно уже
    if app.custom_gamma_window_s2 is not None and app.custom_gamma_window_s2.winfo_exists():
        try:
            app.custom_gamma_window_s2.lift()
            app.custom_gamma_window_s2.focus_force()
        except:
            pass
        return
    
    win = tk.Toplevel(app.root)
    app.custom_gamma_window_s2 = win
    win.title("Custom Gamma S2 - Curve Editor")
    ...
    # Базовый размер окна (ограничиваем максимальным значением и доступным экраном)
    ...
    bg_item = None
    gamma_bg_original2 = None
    try:
        # Use path_utils to resolve background image location
        bg_path = resolve_resource_path("background.png")
        if os.path.exists(bg_path):
            gamma_bg_original2 = Image.open(bg_path).convert("RGBA")
            # Create initial scaled background - используем размеры окна
            initial_width = base_width
            initial_height = base_height
            bg_img = gamma_bg_original2.resize((initial_width, initial_height), Image.Resampling.LANCZOS)
            app.custom_gamma_s2_bg = ImageTk.PhotoImage(bg_img)
            bg_item = main_canvas.create_image(0, 0, image=app.custom_gamma_s2_bg, anchor="nw")
    except Exception as e:
        print(f"[WARN] Failed to load background for custom gamma window S2: {e}")
    ...
    def _resize_background(event):
        if bg_item and gamma_bg_original2:
            try:
                new_width = event.width
                new_height = event.height
                # Stretch background to entire Canvas (in both directions)
                resized_img = gamma_bg_original2.resize((new_width, new_height), Image.Resampling.LANCZOS)
                app.custom_gamma_s2_bg = ImageTk.PhotoImage(resized_img)
                main_canvas.itemconfig(bg_item, image=app.custom_gamma_s2_bg)
            except:
                pass
    ...
    ttk.Radiobutton(
        controls,
        text="RGB",
        variable=app.custom_gamma_rgb_mode2,
        value="rgb",
    ...
    def save_custom_gamma_arrays():
        ...
        try:
            app.saved_custom_gamma_sdr_r2 = np.array(
                app.custom_gamma_sdr_r2[:CUSTOM_GAMMA_POINTS],
                dtype=np.float32,
                copy=True
            )
            app.saved_custom_gamma_sdr_g2 = np.array(
                app.custom_gamma_sdr_g2[:CUSTOM_GAMMA_POINTS],
                dtype=np.float32,
                copy=True
            )
            app.saved_custom_gamma_sdr_b2 = np.array(
                app.custom_gamma_sdr_b2[:CUSTOM_GAMMA_POINTS],
                dtype=np.float32,
                copy=True
            )
        except Exception as e:
    ...
    def rebuild_sliders():
        ...
        for i, s in enumerate(g_sliders):
            val = int(app.custom_gamma_sdr_g2[i])
            s.set(val)
        
        for i, s in enumerate(b_sliders):
            val = int(app.custom_gamma_sdr_b2[i])
            s.set(val)
    ...
    def reset_curve():
        ...
        print("[OK] Custom Gamma S2 reset to default Curve=3.0 Bias=0.0")
    
    def save_custom_gamma():
        """Save custom gamma values to file"""
        path = open_file_dialog(win, "save", title="Save Custom Gamma", filetypes=[("JSON files", "*.json")])
        if not path:
            return
        try:
            import json
            ...
            print(f"[OK] Custom Gamma S2 saved to: {path}")
        except Exception as e:
            print(f"[ERROR] Failed to save Custom Gamma S2: {e}")
    
    def load_custom_gamma():
        """Load 64-point Custom Gamma curve from file."""
        ...
        try:
            ...
            app.custom_gamma_sdr_g2[:] = np.asarray(
                values_g, dtype=np.float32
            )
            app.custom_gamma_sdr_b2[:] = np.asarray(
            ...
            print(f"[ERROR] Failed to load Custom Gamma S2: {e}")
    ...
    g_panel, g_sliders, g_slider_vars, g_draw = create_gamma_panel(body, app.custom_gamma_sdr_g2, "#40ff40")
    ...
    try:
        ...
        if len(app.saved_custom_gamma_sdr_g2) == CUSTOM_GAMMA_POINTS:
            app.custom_gamma_sdr_g2[:] = app.saved_custom_gamma_sdr_g2[:CUSTOM_GAMMA_POINTS]
    ...
    win.bind("<Destroy>", lambda e: setattr(app, 'custom_gamma_window_s2', None))