"""
LED Calibration Window for Stream 1
"""
...
def open_calibration_window(parent_app, calibration: dict):
    """
    Open Stream 1 LED Calibration window
    ...
    if parent_app.calibration_window1 is not None and parent_app.calibration_window1.winfo_exists():
        # If window exists, focus on it
        try:
            parent_app.calibration_window1.lift()
            parent_app.calibration_window1.focus_force()
        except:
            pass
        return
    ...
    try:
        ...
        if os.path.exists(bg_path):
            ...
            bg_img = calib_bg_original1.resize((initial_width, initial_height), Image.Resampling.LANCZOS)
            parent_app.calibration_window1_bg = ImageTk.PhotoImage(bg_img)
            bg_item = main_canvas.create_image(0, 0, image=parent_app.calibration_window1_bg, anchor="nw")
    except Exception as e:
        print(f"[WARN] Failed to load background for calibration window 1: {e}")
    ...
    def _resize_background(event):
        if bg_item and calib_bg_original1:
            try:
                new_width = event.width
                new_height = event.height
                # Stretch background to entire Canvas (in both directions)
                resized_img = calib_bg_original1.resize((new_width, new_height), Image.Resampling.LANCZOS)
                parent_app.calibration_window1_bg = ImageTk.PhotoImage(resized_img)
                main_canvas.itemconfig(bg_item, image=parent_app.calibration_window1_bg)
            except:
                pass
    ...
    lut_combo = ttk.Combobox(
        top,
        textvariable=parent_app.lut_size1,
    ...
    def load_sdr():
        parent_app.load_external_lut(1, "SDR")
    ...
    def load_hdr():
        parent_app.load_external_lut(1, "HDR")
        update_lut_info_labels()
    ...
    interp_sdr_cb = ttk.Checkbutton(top, text="Auto interp. SDR→256", variable=parent_app.interp_sdr_1)
    interp_sdr_cb.pack(side="left", anchor="w", padx=(0, 4))
    
    interp_hdr_cb = ttk.Checkbutton(top, text="Auto interp. HDR→256", variable=parent_app.interp_hdr_1)
    interp_hdr_cb.pack(side="left", anchor="w", padx=(0, 4))

    # LUT info labels (SDR & HDR size with interpolation, internal/external)
    ttk.Separator(top, orient="vertical").pack(side="left", fill="y", padx=(12, 5))
    ...
    def update_lut_info_labels():
        ...
        base_size = max(2, int(parent_app.lut_size1.get()))

        # Check if external SDR LUT is loaded
        ext_sdr = getattr(parent_app, "external_lut_sdr_1", None)
        ...
        if sdr_is_external:
            ...
            if parent_app.interp_sdr_1.get():
                sdr_size = 256
        else:
    ...
    def on_interp_hdr_toggle():
        if parent_app.interp_hdr_1.get():
            ...
            try:
                parent_app.interpolate_lut_to_256(1, "HDR")
            except Exception as e:
                print(f"[WARN] HDR interpolation failed: {e}")
        else:
            # Disabled: restore original external LUT from saved path
            try:
                _restore_external_lut(parent_app, 1, "HDR")
            except Exception as e:
    ...
    def get_lut_size():
        return max(2, int(parent_app.lut_size1.get()))
    ...
    def rebuild_lut():
        ...
        generate_3d_lut_async(
            calibration,
            size=get_lut_size(),
            callback=lambda lut: parent_app.root.after(0, lambda l=lut: setattr(parent_app, 'global_lut', l))
        )
    ...
    win.bind("<Destroy>", lambda e: setattr(parent_app, 'calibration_window1', None))
